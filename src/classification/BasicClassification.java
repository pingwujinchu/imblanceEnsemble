...
//	}
	...
	public EvaluationInfo getResult(String methodname, String classifiername, double validationResult[], int times,EvaluationInfo ei) throws Exception {
		...
		for(int i = 0 ; i < numClass ; i++){
			ei.tpRate[i] = validationResult[i]*100/times;
			pos ++;
		}

		//获取FPR
		for(int i = 0; i < numClass ; i++){
			ei.fpRate[i] = validationResult[i + pos]*100/times;
			pos ++;
		}
		
		//获取precision
		for(int i = 0 ; i < numClass ; i++){
			ei.precision[i] = validationResult[i + pos]*100/times;
			pos ++;
		}
		
		//获取recall
		for(int i = 0 ; i < numClass ; i++){
			ei.recall[i] = validationResult[i + pos]*100/times;
			pos ++;
		}
		
		//获取FMeasure
		for(int i = 0 ; i < numClass ; i++){
			ei.fMeasure[i] = validationResult[i + pos]*100/times;
			pos ++;
		}
...
//	}
	...
	public void updateResult(double validationResult[], Evaluation eval){
		...
		//添加FPR
		for(int i = 0;i < numClass ; i++){
			validationResult[pos + i] += eval.falsePositiveRate(i);
			pos ++;
		}
		
		//添加precision
		for(int i = 0; i < numClass ; i++){
			validationResult[pos + i] += eval.precision(i);
			pos ++;
		}
		...
		for(int i = 0 ; i < numClass ; i++){
			gmean *= eval.recall(i);
			validationResult[pos + i] += eval.recall(i);
			pos ++;
		}
		
		//添加Fmeasure
		for(int i = 0 ; i < numClass ; i++){
			validationResult[pos + i] += eval.fMeasure(i);
			pos ++;
		}
